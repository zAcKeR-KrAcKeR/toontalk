"""
Transcribe the merged 37s Shinchan clip (whisper-large-v3),
upload to VideoDB, and update metadata with new ref_audio_id + ref_text.
"""
import os, json
...
clip_path = AUDIO_DIR / "shinchain_voice_best.mp3"

print(f"File: {clip_path.name} ({clip_path.stat().st_size // 1024} KB)")

# Step 1: Transcribe with whisper-large-v3 (better accuracy than turbo)
...
print(f"ref_text ({len(ref_text)} chars): {ref_text[:150]}")

# Step 2: Upload merged clip to VideoDB
print("\nUploading merged Shinchan clip to VideoDB...")
audio_obj = coll.upload(file_path=str(clip_path), media_type="audio")
print(f"Uploaded: id={audio_obj.id}")
...
meta["character_voices"]["shinchan"] = {
    ...
    "ref_text":      ref_text,
    "source":        "manual_mp3",
    "clip_info":     "29s + 7s merged = 37s clean ref",
}
with open(META_PATH, "w", encoding="utf-8") as f:
    json.dump(meta, f, ensure_ascii=False, indent=2)

print(f"\nDone! Shinchan ref updated:")
